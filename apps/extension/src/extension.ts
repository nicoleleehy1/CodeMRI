...
export function activate(context: vscode.ExtensionContext) {
    ...
    panel.webview.onDidReceiveMessage(async message=>{
      try {
        ...
        if (message.type==='runTests') {
          // Explicit, user-initiated: runs repository test commands (in an isolated copy on the backend), never on a stale graph.
          if(!repoId) throw new Error('Analyze a repository first.');
          if(stale&&!pending) throw new Error('Source changed. Reanalyze before running tests.');
          if(!vscode.workspace.isTrusted) throw new Error('Trust the workspace before running its tests.');
          if(busy||agentRunning||deciding) throw new Error('An operation is already running.');
          const query=String(message.query??'');
          // While a proposal is under review, tests run against the proposed files applied to a fresh copy.
          const files=pending?.files.map(f=>({path:f.path,before:f.before,after:f.after}))??[];
          const choice=await vscode.window.showWarningMessage(`Run the tests CodeMRI associates with "${query.slice(0,80)}"${files.length?` against the ${files.length} proposed file change(s)`:''}? This executes the repository's own test commands in a temporary copy.`,{modal:true},'Run tests');
  ...
  async function chatRun(prompt:string) {
    ...
    try {
      ...
      let pack='';
      if(repoId){
        void send({type:'chatProgress',text:'Compiling CodeMRI context for the agent…'});
        try {
          // Never compile from a stale graph: confirm the source revision first and refresh when it moved.
          const fresh=await api(`/graphs/${repoId}/freshness`);
