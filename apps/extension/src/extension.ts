    ...
    panel.webview.onDidReceiveMessage(async message=>{
      try {
        ...
        if (message.type==='runTests') {
          ...
          if(!repoId) throw new Error('Analyze a repository first.');
          if(stale) throw new Error('Source changed. Reanalyze before running tests.');
          if(!vscode.workspace.isTrusted) throw new Error('Trust the workspace before running its tests.');
          if(busy||agentRunning) throw new Error('An operation is already running.');
          const query=String(message.query??'');
          const choice=await vscode.window.showWarningMessage(`Run the tests CodeMRI associates with "${query.slice(0,80)}"? This executes the repository's own test commands in a temporary copy.`,{modal:true},'Run tests');
          if(choice!=='Run tests') return;
          busy=true; void send({type:'status',message:'Running impacted tests in an isolated copy…'});
          try {
            const result=await api(`/graphs/${repoId}/tests/run`,{query,timeout:Number(vscode.workspace.getConfiguration('codemri').get<number>('testTimeout')||300)});
