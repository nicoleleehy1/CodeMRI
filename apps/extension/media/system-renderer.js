...
export function drawSystem(svg,layout,{activate,inspect,selectedIds,affectedIds,search}){
  svg.replaceChildren();
  const defs=element('defs'),marker=element('marker',{id:'sys-arrow',viewBox:'0 0 10 10',refX:9,refY:5,markerWidth:6,markerHeight:6,orient:'auto'});marker.append(element('path',{d:'M0 0 L10 5 L0 10 Z',fill:'#8294a9'}));defs.append(marker);svg.append(defs);
  ...
  function drawEdges(container,ox=0,oy=0){for(const e of container.edges||[]){const original=e.original;const origin=offsets.get(e.container)||{x:ox,y:oy};const ex=origin.x,ey=origin.y;const g=element('g',{class:'relationship',tabindex:0,role:'button','aria-label':original.label||original.kind});
      for(const section of e.sections||[]){const points=[section.startPoint,...(section.bendPoints||[]),section.endPoint];const d=points.map((p,i)=>`${i?'L':'M'} ${p.x+ex} ${p.y+ey}`).join(' ');g.append(element('path',{d,class:'edge '+(original.inferred?'inferred':''),'marker-end':section.outgoingShape===e.targets?.[0]||!section.outgoingSections?.length?'url(#sys-arrow)':'none'}),element('path',{d,class:'edge-hit'}));}
  ...
  for(const positioned of nodes){const n=positioned.original,tone=Math.max(0,layout.children.findIndex(g=>(g.children||[]).some(c=>c.id===n.id)))%6,w=positioned.width,h=positioned.height;const g=element('g',{transform:`translate(${positioned.x},${positioned.y})`,class:'node system-node'+(selectedIds.has(n.id)?' selected':'')+(affectedIds.has(n.id)?' affected':'')+(search&&!`${n.name} ${n.summary} ${(n.paths||[]).join(' ')}`.toLowerCase().includes(search)?' dim':''),'data-tone':tone,tabindex:0,role:'button','aria-label':n.name});
    ...
    const meta=n.paths?.length?n.paths[0].split('/').pop():n.shape==='circle'?'external actor':'external resource';g.append(element('text',{x:w/2,y:h-13,'text-anchor':'middle',class:'meta'},meta.length>32?meta.slice(0,30)+'…':meta),element('title',{},n.summary||n.name));
    const click=()=>activate(n);g.onclick=click;g.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();click();}};svg.append(g);
  }
}
