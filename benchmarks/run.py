...
def git_dirty(path: Path) -> bool | None:
    """True when the fixture's working tree differs from HEAD, so `git_revision` alone cannot reproduce the run."""
    try:
        out = subprocess.run(['git', '-C', str(path), 'status', '--porcelain', '--', '.'], capture_output=True, text=True, timeout=10, check=True).stdout
    except (OSError, subprocess.SubprocessError):
        return None
...
def results_row(task: dict, record: dict, repeat_index: int, out: Path, agent: list[str] | None, git_rev: str | None, dirty: bool | None = None) -> dict:
    o = record['outcome']
    ...
    return {
        ...
        'agent_command': agent, 'agent_status': o['agent'], 'git_revision': git_rev, 'git_dirty': dirty, 'graph_revision': record['revision'],
...
def run_task(task_path: Path, agent: list[str] | None, repeat: int = 1, variants=VARIANTS, results: Path | None = None,
    ...
    if dirty:
        print(f'warning: {repo} has uncommitted changes; rows are marked git_dirty=true and are not reproducible from git_revision', file=sys.stderr)
    rows = []
    for n in range(1, repeat + 1):
        for variant in variants:
            ...
            row = results_row(task, record, n, out, agent, git_rev, dirty)
            rows.append(row)
